 ...
 */
const isJurisdictionFilled = ( state: string | null, jurisdictionId: string | null ) => {

   if( state == null || !( state in States ) ) {
      return [false, "Unknown"];
   }

   const { noPollWorkersNeeded, name, jurisdictions: stateJurisdictions } = States[state];
   ...
   if( jurisdictionId != null && stateJurisdictions ) {
      ...
      const jurisdictionName = ( Object.keys( stateJurisdictions ).find( key => ( stateJurisdictions[key] + "" ) === jurisdictionId ) || "" );
      return [
         // strip out any extraneous county name suffixes that aren't in the full jurisdiction name
         FullJurisdictions[state].includes( jurisdictionName.replace( /(County|\(Town\)|\(City\)|Parish|Plantation)$/, "" ).trim() ),
         jurisdictionName,
      ];
   }
