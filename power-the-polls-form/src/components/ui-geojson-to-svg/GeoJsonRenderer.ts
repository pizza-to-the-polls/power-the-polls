...
class GeoJsonRenderer {
      ...
      switch (geojson.type) {
         case "FeatureCollection":
            for (let x = 0; x < geojson.features.length; x++) {
               results = [
                  ...results,
                  ...this.convertFeature(geojson.features[x], opt),
               ];
            }
            break;
         ...
         case "GeometryCollection":
            for (let x = 0; x < geojson.geometries.length; x++) {
               results = [
                  ...results,
                  ...this.convertGeometry(geojson.geometries[x], opt),
               ];
            }
            break;
   ...
   private convertFeature<G extends Geometry = Geometry>(
      ...
      const id =
         attrs.id ||
         (feature as any).id ||
         ((feature as any).properties && (feature as any).properties.id
...
function jsonToSvgElement(json: any, type: GeoJsonTypes, opt?: Options) {
   let pointAsCircle = !!(opt && opt.pointAsCircle);
   let ele =
      (type === "Point" || type === "MultiPoint") && pointAsCircle
         ? "<circle"
         : "<path";
   for (let key in json) {
