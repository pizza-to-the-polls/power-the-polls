...
const Options: FunctionalComponent<OptionProps> = ( { choices, inputName, type }, children ) =>
   type === "select"
      ? (
         <label>
            {children}
 ...
 */
const AdditionalInfoForm: FunctionalComponent<{ data: PtpFormData, onSubmit: () => void }> = ( { data, onSubmit } ) => {
      ...
      try {
         ...
         const toActionKit = elements.reduce(
            ( x, el ) => {
               ...
               if( add ) {
                  x[el.name] = el.name in x ? x[el.name] + ", " + el.value : el.value;
               }
   ...
   return ( <Fragment>
      ...
      <form onSubmit={submitForm} style={{ padding: "0" }}>
         ...
         <Options type="radio" choices={["Yes", "No"]} inputName="action_workeday">
            Are you available to work on the Tuesday, November 3rd election? Hours may begin as early as 5:30am and go until at least an hour after polls close at 8pm. Can you commit to working the full day on Election Day?
         </Options>

         <Options type="radio" choices={["Yes", "No"]} inputName="action_waitlist">
            If you are not selected to be a poll worker, would you be willing to serve on a backup waitlist?
         </Options>
         ...
         <Options type="checkbox"
            ...
            inputName="language"
         >
            Some polling locations may require poll workers who speak different languages to help assist voters with limited English proficiency. Do you speak any other languages besides English? Please select all that apply.
         </Options>
         ...
         {data.state === "MI"
            ...
            :
            <Options type="radio" choices={["Yes", "No", "Maybe"]} inputName="action_travel">
         ...
         <Options type="radio" choices={["Yes", "No"]} inputName="action_pastpollworker">
